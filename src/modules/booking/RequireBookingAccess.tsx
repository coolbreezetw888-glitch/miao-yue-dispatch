// 4.6:行事曆總覽頁(4.3)/手動建單(4.4)/標記完成/取消(4.5)的路由守衛。比照模組 4
// RequireServiceItemsAccess.tsx 的既有寫法。
// 判斷邏輯:merchantRole === 'admin' 一律放行;merchantRole === 'agent' 則要求
// useAgentPermission('orders') 回傳 true 才放行。不符合則導回 /app,不顯示這個頁面存在。
//
// 這個元件只是體驗層的路由守衛,不是安全邊界——真正擋住未授權操作的是
// private.can_manage_bookings 這支函式落實的 RLS/SECURITY DEFINER 函式權限檢查(規格書 3.1/3.3-3.6),
// 即使有人繞過前端路由直接呼叫 API,也會被資料庫擋下(規則 2.12)。
...
export function RequireBookingAccess({ children }: { children: ReactNode }) {
