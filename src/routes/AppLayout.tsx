...
//   2. 渲染輕量化的品牌列(只留 LOGO/品牌名稱,不保留原本那條寬版頂端列——因為主要導覽已經下放到
//      底部分頁籤)。
//   3. 渲染底部分頁籤列:首頁 / 功能 / 行事曆,3 個分頁籤永遠固定顯示,不因角色隱藏整個分頁籤
//      (規格書「新外殼結構」一節明講的刻意設計,分頁籤本身常駐,權限只決定分頁籤「裡面」顯示
//      什麼)。在所有螢幕寬度都套用同一種底部分頁籤外殼,不做手機/桌面兩種版型。
//   4. 透過 <Outlet context={...}> 把 email/登出函式往下傳給「首頁」分頁籤使用,子路由不用
//      重新呼叫一次登入驗證。
//
...
import { useClearCurrentMerchantSelection, useGroupMerchants } from "@/modules/merchant/context";
...
export default function AppLayout() {
  ...
  const outletContext: AppLayoutContext = { email, onSignOut: handleSignOut };
