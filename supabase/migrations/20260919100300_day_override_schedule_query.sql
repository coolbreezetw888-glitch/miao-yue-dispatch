-- 模組 6:訂單管理(第二批)— §5.5 第 3 點:get_merchant_day_schedule 擴充回傳單日例外區間。
-- 每位服務人員的回傳資料新增 availability_overrides 陣列,把相鄰同值的半小時格子合併成一個個
-- {start_time, end_time, is_available} 區間,方便前端渲染,不用回傳一整排零散的半小時格子。
-- 其餘欄位/邏輯完全沿用建單功能擴充 4.2 版本,不變動。
...
begin
  ...
  from (
    select jsonb_build_object(
      ...
      -- 模組 6 §5.5 第 3 點(新增):這位服務人員這一天的單日例外設定,合併相鄰同值的半小時格子
...
comment on function public.get_merchant_day_schedule(uuid, date) is '對應規格書 3.6/5.3,建單功能擴充 4.2,模組 6 §5.5 第 3 點:給行事曆總覽頁使用,回傳當天(Asia/Taipei)所有在職服務人員的可預約邊界(第一層∩第二層,不含第三層——第三層例外另外用 availability_overrides 陣列回傳,由前端疊加判斷最終顯示狀態,見 §5.5 第 4 點)、本店預約明細(含多服務項目陣列、main/assistant 角色標示)、跨商家占用概況(不洩漏對方客戶/商家細節)、單日例外區間(合併相鄰同值半小時格子)。回傳 jsonb。';
