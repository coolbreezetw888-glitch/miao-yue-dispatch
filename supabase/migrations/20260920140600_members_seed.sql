-- 模組 10:會員與紅利 — 種子函式 + create_group_and_merchant/create_merchant_in_group 疊加
-- (第七支)。對應規格書 §3.15。
--
...
-- 取得的(2026-09-20 查證),跟本機 20260920120500_payroll_seed.sql 裡的版本逐字相同(已比對
-- 確認,20260920120500 之後沒有任何 migration 再動過這兩支函式)。以下逐字保留原本函式主體,
-- 只在既有 seed_default_leave_deduction_rules 呼叫之後,補上一行新的 perform 呼叫。這兩支函式
-- 簽章完全沒變(只是疊加一行 perform),不需要 drop function,直接 create or replace 即可。

-- =========================================================================
-- seed_default_member_settings(p_merchant_id uuid):寫入一筆 merchant_member_settings
-- (全部採用 §1.1 的預設值),on conflict (merchant_id) do nothing 保持冪等。
-- =========================================================================
create or replace function public.seed_default_member_settings(p_merchant_id uuid)
returns void
language plpgsql
security definer
...
as $$
begin
  insert into public.merchant_member_settings (merchant_id)
  values (p_merchant_id)
  on conflict (merchant_id) do nothing;
end;
$$;

comment on function public.seed_default_member_settings(uuid) is '模組 10 會員與紅利:新商家建立當下種入一筆預設會員設定(§1.1 的預設值:phone_required_to_create=true、其餘皆 0/false),商家可以之後自己在會員系統設定頁調整。on conflict (merchant_id) do nothing 保持冪等,只在 create_group_and_merchant/create_merchant_in_group 建立商家當下呼叫。';

-- =========================================================================
-- create_group_and_merchant/create_merchant_in_group 補呼叫。逐字保留自
-- 20260920120500_payroll_seed.sql(已對正式環境核對過,無漂移),只新增一行 perform 呼叫,
-- 緊接在既有 seed_default_leave_deduction_rules 之後。
-- =========================================================================
...
comment on function public.create_group_and_merchant(text, text, text, text, text) is 'Onboarding(4.1)呼叫的 RPC:原子性建立集團+第一間商家+登記建立者為管理員+套用產業預設功能+種入模組 9 v2 預設付款方式+種入模組 7 預設假別+種入模組 8 預設薪資設定與假別扣款規則+種入模組 10 預設會員設定,見規格書 3.2、支付方式.md §2、排班與休假管理.md §3.9、薪資與帳務.md §3.12、會員與紅利.md §3.15。';
...
comment on function public.create_merchant_in_group(uuid, text, text, text, text, text) is '新增分店流程(4.4)呼叫的 RPC:檢查規則 2.5 權限後,原子性建立新分店+登記建立者為管理員+套用產業預設功能+種入模組 9 v2 預設付款方式+種入模組 7 預設假別+種入模組 8 預設薪資設定與假別扣款規則+種入模組 10 預設會員設定,見規格書 3.3、支付方式.md §2、排班與休假管理.md §3.9、薪資與帳務.md §3.12、會員與紅利.md §3.15。';
