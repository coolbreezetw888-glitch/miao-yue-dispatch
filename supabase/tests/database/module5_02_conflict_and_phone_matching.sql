...
--
-- 情境布置:兩間不同商家(不同集團)各自有一位服務人員,電話號碼相同(同一人跨店服務),
-- 藉此驗證跨商家電話比對邏輯(同一支電話出現在兩間商家的服務人員名單裡,視為同一人)。
-- 兩間商家都設定週二整天營業(00:00-23:59,避免邊界檢查干擾,聚焦測試重疊/電話比對本身),
-- 服務人員都 no_time_slot_limit=true(不用另外布置 staff_availability_windows)。
--
-- 2026-09-22 補充說明(對應 .project/SPECS-INDEX.md #595/#596):merchant_staff.phone 這批
-- 改成 NOT NULL + CHECK(phone ~ '^09\d{8}$')之後,資料庫層不再可能存下「09xx-xxx-xxx」這種
-- 帶符號的格式,也不可能存 NULL——原本這裡用來驗證「不同格式字串正規化後相同」「電話為 NULL
-- 一律不比對」這兩段情境,在資料庫層已經變成不可能發生的狀態(不是應用邏輯的事,是寫入當下
-- 就會被 CHECK 擋下)。因此原本的兩位「電話格式不同但正規化後相同」師傅改成「電話字串本身就
-- 完全相同」(這才是資料庫現在唯一能達成的「同一人跨店」表示方式),原本的兩位「電話 NULL」
-- 師傅改成「電話不同的兩個人」(用已經有的④號情境同樣的邏輯驗證「電話不同、不互相影響」,
-- 不再測試 NULL 分支——NULL 分支在應用層 create_booking() 函式裡的防呆程式碼還在,只是資料庫
-- 現在保證這個分支不會再被觸發到,不需要、也無法再用 pgTAP fixture 重現)。
begin;
...
-- 一店的 A 師傅、二店的 A 師傅(同一人跨店服務),電話字串完全相同:0912345678。
-- 兩人都 no_time_slot_limit=true,避免另外布置時段。
insert into merchant_staff (id, merchant_id, name, phone, no_time_slot_limit) values
  ('b2000000-0000-4000-8000-000000000041', 'b2000000-0000-4000-8000-000000000021', 'A師傅(一店)', '0912345678', true),
...
insert into merchant_staff (id, merchant_id, name, phone, no_time_slot_limit) values
  ('b2000000-0000-4000-8000-000000000043', 'b2000000-0000-4000-8000-000000000021', 'B師傅(一店)', '0922000000', true);

-- 二店另一位師傅 C,電話跟一店的 D 師傅不同,用來驗證「電話不同的兩人,不互相影響」
-- (原本這裡測的是「電話 NULL」分支,見檔案開頭 2026-09-22 補充說明,NULL 分支現在資料庫層
-- 已經不可能發生,改成測「電話不同」這個仍然有效、仍然會發生的情境)。
insert into merchant_staff (id, merchant_id, name, phone, no_time_slot_limit) values
  ('b2000000-0000-4000-8000-000000000044', 'b2000000-0000-4000-8000-000000000022', 'C師傅(二店)', '0933900001', true);
-- 一店也放一位電話不同的 D 師傅,對照 C。
insert into merchant_staff (id, merchant_id, name, phone, no_time_slot_limit) values
...
-- ⑤ 電話不同的兩邊(D 師傅 vs C 師傅)不互相比對——先幫 D 師傅建一筆預約
--   (原本這裡測「電話 NULL」分支,見檔案開頭 2026-09-22 補充說明)。
select lives_ok(
  $$select create_booking(
...
-- ⑥ 二店幫電話不同的 C 師傅在同一時段建立預約,不應該被擋下(電話不同,不比對)。
select pg_temp.test_set_auth('b2000000-0000-4000-8000-000000000002');
